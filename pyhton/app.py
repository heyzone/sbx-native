...
UUID = os.environ.get('UUID', 'a87056c0-abeb-45e4-a97e-f23bdf84d191') # 节点和哪吒v1使用的UUID，默认固定值，建议自行生成一个唯一的UUID
...
ARGO_PORT = int(os.environ.get('ARGO_PORT', '34573')) # 隧道端口,使用固定隧道token时需要在cloudflare里设置和这里一致
ARGO_DOMAIN = os.environ.get('ARGO_DOMAIN', 'hgf3.aser.kdns.fr')  # 固定隧道域名，留空将使用临时隧道
ARGO_AUTH = os.environ.get('ARGO_AUTH', 'eyJhIjoiNTk5MzUwOTkyOTQzNmJkYzVhNTdmYjJmN2Y5YTlkMjAiLCJ0IjoiNGZlOGY3MzAtZGM3OS00MTE3LTlmNjItNWFmYzMxYWNlYzA5IiwicyI6Ill6ZGxZalUxTVdFdE0yRXdOaTAwTkdNeExXSTJZV1V0T0RjNU16RTJaVGMwWVRkaSJ9')      # 固定密钥token或json，留空将使用临时隧道
...
PORT = int(os.environ.get('PORT', '7860'))       # HTTP服务器端口，默认3000,用于提供订阅和前端伪装页
NAME = os.environ.get('NAME', 'hgf3')               # 节点名称前缀
